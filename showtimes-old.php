...
$seasonquery = "SELECT show_season_id, show_season_name FROM shows_season";
$seasonresult = mysqli_query($conn, $seasonquery);

//Placing Show_Season_id into an array
$seasonarray = array();
while($row = mysqli_fetch_array($seasonresult)){
    $show_season_id = $row["show_season_id"];
    //Setting key in array - Specifies the key (numeric or string)
    //                            sets the value - Specifies the value
    $seasonarray[$show_season_id] = $row['show_season_name'];
